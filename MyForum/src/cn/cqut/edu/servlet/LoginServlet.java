...
public class LoginServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		int state;
		System.out.println("方式："+actionType);
		if(actionType == null){	// 前台登陆
			...
			state = Integer.parseInt(loginResult.get(0).toString());//here
		...
		out.flush();
		out.close();
		request.getRequestDispatcher("/admin/adminIndex.jsp").forward(request, response);//跳转
