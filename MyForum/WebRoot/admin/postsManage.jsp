	...
	<script type="text/javascript">
		...
		function setData(posts) {
			...
			for(var i = 0 ; i < posts.length ; i++) {
				console.log( posts[i].publishTime.time);
				//var j = JSON.parse(posts[i].publishTime);
				//var Rtime =  eval(posts[i].publishTime.time);
				//Rtime = Rtime /1000;
				//Rtime=time.formatTime(Rtime,'Y/M/D h:m:s');//转换为日期：2017/03/03 03:03:03
				//var Rtime = new Date(posts[i].publishTime.time);
				var Rtime = formatTime(posts[i].publishTime.time,'Y/M/D h:m:s');
				
				str += "<tr>"
						+ "<td style='text-align:center'>" + posts[i].title.substring(0,10) + "</td>"
						+ "<td style='text-align:center'>" + Rtime +"</td>"
